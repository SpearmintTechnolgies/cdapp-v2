      ...
      <div className={" mb-2"}>
        ...
        >
          ...
          <div className="d-flex align-items-center gap-2">
            {!show ? <button className="btn1" onClick={() => setBuyValue(tokenData?.isToken == true
              ? formatEther(alternateTokenBalance || 0)
              : parseFloat(balance?.value > (TOTAL_RAISE -  projects?.[6]) ? formatEther((TOTAL_RAISE -  projects?.[6]) || 0) : formatEther(balance?.value || 0)).toFixed(18))}>Max</button> : null}
        ...
        {!show ? <div className={style.btn_group+" d-flex gap-2 mt-2"} > 
          ...
          <div className={style.btn_group+" d-flex gap-2 mt-2"} >
            ...
            <button className=" px-1" style={{ fontSize: "14px" }} onClick={() => setSellValue(parseFloat(formatEther(tokenBalance) * 2).toFixed(18))}>75 %</button>
            <button className=" px-1" style={{ fontSize: "14px" }} onClick={() => setSellValue(parseFloat(formatEther(tokenBalance) * 1).toFixed(18))}>100 %</button>
          </div>
        }
        {buyErr && (
          <Typography
...
https://github.com/himotechglobal/cdapp-v2.git