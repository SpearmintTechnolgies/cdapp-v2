        ...
        >
          ...
          <div className="d-flex align-items-center gap-2">
            {!show ? <button className="btn1" onClick={() => setBuyValue(tokenData?.isToken == true
              ? formatEther(alternateTokenBalance || 0)
              : parseFloat(balance?.value > (TOTAL_RAISE -  projects?.[6]) ? formatEther((TOTAL_RAISE -  projects?.[6]) || 0) : parseFloat(formatEther(balance?.value || 0) - (formatEther(balance?.value || 0)*0.02) )).toFixed(18))}>Max</button> : null}
